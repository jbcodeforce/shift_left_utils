...
class TestPipelineWorker(unittest.TestCase):
    ...
    def test_create_pipelines_from_fact(self):
        ...
        inventory=get_or_build_inventory_from_ddl(pipeline_folder, pipeline_folder, True)
        hierarchy=build_pipeline_definition_from_table(pipeline_folder + "/facts/p1/fct_order/sql-scripts/dml.fct_order.sql", [], inventory)
        assert hierarchy
