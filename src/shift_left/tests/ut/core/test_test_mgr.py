...
class TestTestManager(unittest.TestCase):
    ...
    def test_process_foundation_ddl_missing_foundation_no_dml(self):
        """Missing foundation and no dml_content raises ValueError."""
        foundation = Foundation(table_name="missing_table", ddl_for_test="./tests/ddl_missing_table.sql")
        test_definition = SLTestDefinition(foundations=[foundation], test_suite=[])
        table_inventory = {}
        with self.assertRaises(ValueError) as ctx:
            test_mgr._process_foundation_ddl_from_test_definitions(
                test_definition, "/tmp/tests", table_inventory, dml_content=None
            )
        self.assertIn("missing_table", str(ctx.exception))
        self.assertIn("not in table_inventory", str(ctx.exception))

    @patch('shift_left.core.test_mgr.from_pipeline_to_absolute')
    @patch('shift_left.core.test_mgr.SQLparser')
    def test_process_foundation_ddl_missing_foundation_with_dml(self, mock_parser_class, mock_from_pipeline):
        """When foundation is not in inventory, DDL is derived from dml_content."""
        mock_parser = mock_parser_class.return_value
        mock_parser.build_column_metadata_from_dml_select.return_value = {
            "a": {"name": "a", "type": "STRING", "nullable": True, "primary_key": False},
            "b": {"name": "b", "type": "BIGINT", "nullable": True, "primary_key": False},
        }
        mock_parser.build_create_table_from_column_metadata.return_value = (
            "CREATE TABLE IF NOT EXISTS missing_foundation_ut (\n    a STRING,\n    b BIGINT\n)"
        )
        foundation = Foundation(table_name="missing_foundation", ddl_for_test="./tests/ddl_missing_foundation.sql")
        test_definition = SLTestDefinition(foundations=[foundation], test_suite=[])
        table_inventory = {}
        dml_content = "INSERT INTO target SELECT a, b FROM missing_foundation"
        test_dir = "/tmp/tests_dml_fallback"
        os.makedirs(test_dir, exist_ok=True)
        ddl_dir = os.path.normpath(os.path.join(test_dir, "..", "tests"))
        os.makedirs(ddl_dir, exist_ok=True)
        try:
            result = test_mgr._process_foundation_ddl_from_test_definitions(
                test_definition, test_dir, table_inventory, dml_content=dml_content
            )
            self.assertIn("missing_foundation", result)
            self.assertEqual(result["missing_foundation"]["a"]["type"], "STRING")
            self.assertEqual(result["missing_foundation"]["b"]["type"], "BIGINT")
            mock_parser.build_column_metadata_from_dml_select.assert_called_once_with(
                dml_content, "missing_foundation"
            )
            ddl_out_abs = os.path.normpath(os.path.join(test_dir, "..", "tests", "ddl_missing_foundation.sql"))
            self.assertTrue(os.path.exists(ddl_out_abs), f"Expected DDL file at {ddl_out_abs}")
            with open(ddl_out_abs) as f:
                content = f.read()
            self.assertIn("missing_foundation_ut", content)
            self.assertIn("a STRING", content)
        finally:
            ddl_clean = os.path.normpath(os.path.join(test_dir, "..", "tests", "ddl_missing_foundation.sql"))
            if os.path.exists(ddl_clean):
                os.remove(ddl_clean)
            if os.path.exists(test_dir):
                import shutil
                shutil.rmtree(test_dir)
            parent_tests = os.path.normpath(os.path.join(test_dir, "..", "tests"))
            if os.path.exists(parent_tests) and not os.listdir(parent_tests):
                os.rmdir(parent_tests)
