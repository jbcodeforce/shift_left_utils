...
class TestKsqlCodeAgent(unittest.TestCase):
    ...
    @patch('builtins.input')
    def test_validation_fails_max_iterations(self, mock_input):
    ...
    @patch('builtins.input')
    def test_user_stops_early_after_first_failure(self, mock_input):
    ...
    @patch('builtins.input')
    def test_user_stops_early_after_second_failure(self, mock_input):
    ...
    @patch('builtins.input')
    def test_user_stops_immediately_on_success(self, mock_input):
    ...
    @patch('builtins.input')
    def test_agent_history_tracking(self, mock_input):
    ...
    def test_print_statements_called_correctly(self, mock_print, mock_input):
        """Test that print statements are called with correct messages."""
        # Mock validation to fail once, succeed second time
        self.agent._validate_flink_sql_on_cc = MagicMock(side_effect=[
        ...
        _iterate_on_validation(self.agent, self.test_sql)
        
        # Verify print calls
        print_calls = [call[0][0] for call in mock_print.call_args_list]
        
        # Should print error message
        self.assertIn(f"\n\n--> Error: {self.error_message}", print_calls)
        # Should print refinement agent message
        self.assertIn("Process with refinement agent 1", print_calls)
        # Should print refined SQL
        self.assertIn(f"Refined sql:\n {self.refined_sql}", print_calls)
        # Should print continuation prompt
        self.assertIn("do you want to continue? (y/n) or you continue with the generated sql?", print_calls)
    
    @patch('builtins.input')
    def test_edge_case_empty_sql(self, mock_input):
    ...
    @patch('builtins.input')
    def test_edge_case_various_user_inputs(self, mock_input):
    ...
    @patch('builtins.input')
    def test_translate_without_validation(self, mock_input):
    ...
    def test_translate_with_validation_user_declines(self, mock_print, mock_input):
        ...
        # Verify validation prompt was printed
        print_calls = [call[0][0] for call in mock_print.call_args_list]
        self.assertIn("Start validating the flink SQLs, do you want to continue? (y/n)", print_calls)
    
    @patch('shift_left.core.utils.ksql_code_agent._iterate_on_validation')
    @patch('builtins.input')
    def test_translate_with_validation_full_success(self, mock_input, mock_iterate_validation):
        ...
        mock_iterate_validation.assert_any_call(self.agent, "VALIDATED_DML")
    ...
    @patch('shift_left.core.utils.ksql_code_agent._iterate_on_validation')
    @patch('builtins.input')
    def test_translate_with_validation_ddl_success_dml_fail(self, mock_input, mock_iterate_validation):
    ...
    @patch('shift_left.core.utils.ksql_code_agent._iterate_on_validation')
    @patch('builtins.input')
    def test_translate_with_validation_ddl_fails(self, mock_input, mock_iterate_validation):
    ...
    @patch('builtins.input')
    @patch('builtins.print')
    def test_translate_print_statements(self, mock_print, mock_input):
        """Test that appropriate print statements are made during translation."""
        # Mock the agent methods
        self.agent._translator_agent = MagicMock(return_value=("DML_OUTPUT", "DDL_OUTPUT"))
        self.agent._mandatory_validation_agent = MagicMock(return_value=("VALIDATED_DML", "VALIDATED_DDL"))
        
        ksql_input = "CREATE STREAM test AS SELECT * FROM source"
        
        # Test without validation
        self.agent.translate_from_ksql_to_flink_sql(ksql_input, validate=False)
        
        # Verify print calls
        print_calls = [call[0][0] for call in mock_print.call_args_list]
        
        # Should print translator agent completion
        translator_print = "Done with translator agent, the flink sql is:\n DML_OUTPUT\nDDL_OUTPUT"
        self.assertIn(translator_print, print_calls)
        
        # Should print mandatory validation completion
        mandatory_print = "Done with mandatory validation agent updated flink sql:\n VALIDATED_DML\n"
        self.assertIn(mandatory_print, print_calls)
    
    @patch('builtins.input')
    def test_translate_edge_cases(self, mock_input):
    ...
    @patch('shift_left.core.utils.ksql_code_agent._iterate_on_validation')
    @patch('builtins.input')
    def test_translate_method_call_order(self, mock_input, mock_iterate_validation):
