...
class TestDeploymentManager(unittest.TestCase):
    ...
    @patch('shift_left.core.utils.ccloud_client.ConfluentCloudClient.post_flink_statement')
    @patch('shift_left.core.deployment_mgr.delete_statement_if_exists')
    def test_clean_things(self, mock_confluent_client, mock_delete):

        mock_client_instance = MagicMock()
        mock_confluent_client.return_value = mock_client_instance
        mock_client_instance.post_flink_statement.return_value = {"status": "success"}
        mock_delete.return_value = None

        for table in ["src_table_1", "src_table_2", "src_table_3", "int_table_1", "int_table_2", "fct_order"]:
            dm.drop_table(table)
            mock_client_instance.post_flink_statement.assert_called_once_with(
                'lfcp-xvrvmz',
                "drop-" + table.replace('_','-'),
                f"drop table {table};",
                {'sql.current-catalog': 'j9r-env', 'sql.current-database': 'j9r-kafka'}
            )
