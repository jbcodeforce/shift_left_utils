...
class TopicListCache(BaseModel):
    ...
    topic_list: List[str] = Field(default=[])
...
def init_unit_test_for_table(table_name: str,
        ...
        use_ai: bool = False,
        post_fix_unit_test: str = DEFAULT_POST_FIX_UNIT_TEST) -> None:
    """
...
def delete_test_artifacts(table_name: str,
                          compute_pool_id: Optional[str] = None,
                          post_fix_unit_test: str = "_ut") -> None:
    """
...
def _execute_flink_test_statement(
    ...
    is_new = False
    if existing_statement is None or isinstance(existing_statement, StatementError):
        should_execute = True
        is_new = True
    elif isinstance(existing_statement, Statement):
        # Statement exists - check if it's running
        if existing_statement.status and existing_statement.status.phase != "RUNNING":
            should_execute = True
            statement_mgr.delete_statement_if_exists(statement_name)
        else:
            logger.info(f"{statement_name} statement already exists -> do nothing")
            return existing_statement, is_new  # Return existing statement, not new
    ...
    if should_execute:
        ...
        if isinstance(existing_statement, Statement):
            logger.info(f"{existing_statement.name} statement already exists -> {existing_statement.status.phase}")
            print(f"{statement_name} statement already exists -> {existing_statement.status.phase}")
            return existing_statement, is_new
        else:
...
def _start_ddl_dml_for_flink_under_test(table_name: str,
    ...
    """
    def replace_table_name(sql_content: str, table_name: str, post_fix: str) -> str:
        """
        ...
        for table in sorted_table_names:
            ...
            if table+post_fix in sql_content:
                sql_content = re.sub(backtick_pattern, f'`{table}{post_fix}`', sql_content, flags=re.IGNORECASE)
            else:
...
def _execute_test_validation(test_case: SLTestCase,
    ...
    for output_step in test_case.outputs:
        ...
        try:
            ...
            if delete_result and "deleted" not in delete_result:
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"Error in final cleanup of statement {statement_name}: {cleanup_error}")
...
def _add_test_files(table_to_test_ref: FlinkTableReference,
                    ...
                    nb_test_cases: int = DEFAULT_TEST_CASES_COUNT,
                    use_ai: bool = False,
                    post_fix_unit_test: str = DEFAULT_POST_FIX_UNIT_TEST) -> SLTestDefinition:
    """
    ...
    table_struct = _process_foundation_ddl_from_test_definitions(
        test_definition, tests_folder_path, table_inventory, dml_content=sql_content
    )
        ...
        for input_data in test_case.inputs:
            if input_data.file_type == "sql":
                ...
                with open(input_file, "w") as f:
                    f.write(f"insert into {input_data.table_name}{post_fix_unit_test}\n({columns_names})\nvalues\n{rows}\n")
...
def _process_foundation_ddl_from_test_definitions(
    test_definition: SLTestDefinition,
    tests_folder_path: str,
    table_inventory: Dict[str, FlinkTableReference],
    dml_content: Optional[str] = None,
) -> Dict[str, Dict[str, Any]]:
    """
    Create a matching DDL statement for the referenced tables. Get the table columns structure for future
    data generation step. Save the DDL for unit test to the tests folder.
    When a foundation table is not in table_inventory, derive DDL from dml_content (table-under-test's DML) if provided.
    """
    table_struct: Dict[str, Dict[str, Any]] = {}  # table_name -> {column_name -> column_metadata}
    parser = SQLparser()
    for foundation in test_definition.foundations:
        if foundation.table_name in table_inventory:
            input_table_ref: FlinkTableReference = FlinkTableReference.model_validate(
                table_inventory[foundation.table_name]
            )
            file_path = from_pipeline_to_absolute(input_table_ref.ddl_ref)
            with open(file_path, "r") as f:
                ddl_sql_content = f.read()
            columns = parser.build_column_metadata_from_sql_content(ddl_sql_content)
            ddl_sql_content = ddl_sql_content.replace(
                input_table_ref.table_name,
                f"{input_table_ref.table_name}{DEFAULT_POST_FIX_UNIT_TEST}",
            )
            ...
            logger.debug(f"Foundation {foundation.table_name} DDL taken from inventory")
        else:
            if not dml_content:
                raise ValueError(
                    f"Foundation table {foundation.table_name} is not in table_inventory "
                    "and dml_content was not provided to derive DDL."
                )
            columns = parser.build_column_metadata_from_dml_select(dml_content, foundation.table_name)
            if not columns:
                raise ValueError(
                    f"Foundation table {foundation.table_name} is not in table_inventory "
                    "and DDL could not be derived from dml_content (no SELECT list parsed)."
                )
            ddl_sql_content = parser.build_create_table_from_column_metadata(
                foundation.table_name, columns, table_name_suffix=DEFAULT_POST_FIX_UNIT_TEST
            )
            ddl_file = os.path.join(tests_folder_path, "..", foundation.ddl_for_test)
            ddl_dir = os.path.dirname(ddl_file)
            if ddl_dir:
                os.makedirs(ddl_dir, exist_ok=True)
            with open(ddl_file, "w") as f:
                f.write(ddl_sql_content)
            table_struct[foundation.table_name] = columns
            logger.info(f"Foundation {foundation.table_name} DDL derived from DML (not in inventory)")
    return table_struct
...
def _coerce_cache_created_at(created_at) -> Optional[datetime]:
    """Normalize cache timestamp from Pydantic (datetime) or JSON file (ISO / legacy str)."""
    ...
    if _topic_list_cache == None:
        ...
        if os.path.exists(TOPIC_LIST_FILE):
            try:
                ...
                cache_created_at = _coerce_cache_created_at(_topic_list_cache.created_at)
                if cache_created_at and (datetime.now() - cache_created_at).total_seconds() < get_config()['app']['cache_ttl']:
                    reload = False
            except Exception as e:
                logger.info(f"Re-loading topic list from file {TOPIC_LIST_FILE}: {e}")
                reload = True
...
def _replace_table_name_ut_with_configured_postfix(sql_content: str, table_name: str, post_fix: str) -> str:

    return sql_content.replace(table_name+DEFAULT_POST_FIX_UNIT_TEST, f"{table_name}{post_fix}")
