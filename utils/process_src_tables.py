...
def _create_file_if_not_exist(new_file):
...
def _create_target_folder(target_root_folder: str) -> str:
    """
    For the given target root folder, creates the structure for the 
    sources subfolder, returns the new source folder path.
    """
    logging.debug(f"Create target folder {target_root_folder}")
...
def select_src_sql_file_processing(sql_file_path: str, generated_code_folder_name: str, walk_parent: bool = False, config: dict = {}):
    ...
    """
    logging.debug(f"\t select_src_sql_file_processing, process file: {sql_file_path}")
    if sql_file_path.find("source") > 0:
        _process_src_sql_file(sql_file_path, generated_code_folder_name, config)
    else:
...
def process_files_in_folder(args):
    ...
    source_target_path = _create_target_folder(args.pipeline_folder_path)
...
def process_one_file(src_file: str, target_folder: str, process_dependency: bool = False, config: dict = {}):
    ...
    generated_code_folder_name = _create_target_folder(target_folder)
...
if __name__ == "__main__":
    ...
    _create_file_if_not_exist(TABLES_TO_PROCESS)
    TABLES_DONE=f"{REPORT_DIR}/tables_done.txt"
    _create_file_if_not_exist(TABLES_DONE)
    if args.folder_path:
